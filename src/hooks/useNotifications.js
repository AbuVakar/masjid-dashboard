...
export const useNotifications = () => {
  ...
  const schedulePrayerNotifications = useCallback(async (times, prefs = notifyPrefs) => {
    try {
  ...
  const sendServiceWorkerNotification = useCallback(async (title, body, options = {}) => {
    try {
      if (!('serviceWorker' in navigator) || !navigator.serviceWorker.controller) {
  ...
  const notify = useCallback(async (title, body, options = {}) => {
    try {
      ...
      const categoryEnabled = notifyPrefs[category] !== undefined ? notifyPrefs[category] : true;
      if (!categoryEnabled) {
      ...
      const priorityEnabled = notifyPrefs.priorityLevel === 'urgent' || 
                            notifyPrefs.priorityLevel === priority ||
                            (notifyPrefs.priorityLevel === 'high' && priority !== 'urgent') ||
                            (notifyPrefs.priorityLevel === 'normal' && ['low', 'normal'].includes(priority));
      ...
      if (notifyPrefs.quietEnabled && priority !== 'urgent') {
        ...
        const [startHour, startMin] = notifyPrefs.quietStart.split(':').map(Number);
      ...
      const enhancedOptions = {
        ...
        vibrate: notifyPrefs.vibrationEnabled && vibration ? [200, 100, 200] : undefined,
        data: {
          type,
          priority,
          category,
  ...
  const notifyEmergency = useCallback((title, body) => {
  ...
  const notifyDataBackup = useCallback((title, body) => {
    return notify(title, body, { 
      type: 'backup', 
      priority: 'normal', 
      category: 'dataBackup' 
  ...
  const notifySystemUpdate = useCallback((title, body) => {
    return notify(title, body, { 
      type: 'system', 
      priority: 'high', 
  ...
  const notifyCommunityEvent = useCallback((title, body) => {
    return notify(title, body, { 
      type: 'event', 
      priority: 'normal', 
